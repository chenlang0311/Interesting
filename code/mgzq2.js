...
function getMoney(data) {
  ...
  let routes = [];
  let allMoney = fMoney;
  console.log("targetData",moreIndexs,maxMoney);
  moreIndexs.forEach((el,i)=>{
    let currentPosition = el
    let end = moreIndexs[i + 1];
    if(end){
      while(!(currentPosition.x == end.x && currentPosition.y == end.y) && todoPoints.length){
